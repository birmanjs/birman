...
test('with src', () => {
  expect(createDefaultConfig(join(fixtures, 'normal'), {}).collectCoverageFrom).toContain(
    'src/**/*.{js,jsx,ts,tsx}'
  );
});
...
test('lerna without package', () => {
  expect(createDefaultConfig(join(fixtures, 'lerna'), {}).collectCoverageFrom).toContain(
