...
function getMockConsoleFunction(name: ConsoleFunctionName): MockConsoleFunction<MessageStore> {
  return function mockConsoleFunction(messageStore: MessageStore = []) {
    const messages = Array.isArray(messageStore) ? messageStore : [];
    const callback = Array.isArray(messageStore) ? null : messageStore;

    const consoleSpy = jest.spyOn(console, name).mockImplementation((...args: any[]) => {
      messages.push(args);
