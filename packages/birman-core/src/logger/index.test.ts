import Logger from './';
import BirmanError from './birman-error';
...
describe('Logger', () => {
  ...
  it('warn', () => {
    ...
    const spy = jest.spyOn(global.console, 'warn').mockImplementation(warnMockFn);
  ...
  it('no debug', () => {
    ...
    const spy = jest.spyOn(process.stderr, 'write').mockImplementation(debugMockFn);
    // jest.spyOn()
    const logger = new Logger('birman-core-debug');
    logger.debug('aaa');
  ...
  describe('error', () => {
    it('normal error', () => {
      ...
      const spy = jest.spyOn(global.console, 'error').mockImplementation(errorMockFn);
    ...
    it('birman error', () => {
      ...
      const logger = new Logger('error');
      // Birman Error
      const birmanError = new BirmanError({
        code: 'ERR_CORE_PLUGIN_RESOLVE_FAILED',
        message: 'test'
      });
      logger.error(birmanError);
      expect(errorMockFn).toBeCalled();
      expect(birmanError.code).toEqual('ERR_CORE_PLUGIN_RESOLVE_FAILED');
      spy.mockRestore();
    });
  });
});
